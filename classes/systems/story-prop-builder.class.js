...
import { STORY_PROP_CONFIGS } from "../../js/config/story-prop-config.js";

const STORY_PLAN = Object.freeze([
  clue("abandonedCompanionCradle", "scrapyard", 146500,
    "route-scrapyard-13", "Lumas verlassene Ladestation"),
  clue("sealedLumaTransport", "factory", 116000,
    "route-factory-128", "Luma wurde durch die Fabrik transportiert"),
  clue("launchTraceConsole", "launch-tower", 86000,
    "route-launch-tower-232", "Der Transport flog zum Mond"),
  clue("stationDetentionPod", "space-station", 56000,
    "route-space-station-323", "Luma war auf der Station gefangen"),
  clue("fortressRouteBeacon", "moon", 26000,
    "route-moon-405", "Die Spur fuehrt zur Waechterfestung"),
  clue("lumaContainmentCapsule", "moon", 600,
    "moon-warden-arena-floor-left", "Luma wartet in der Bossarena"),
]);
...
export class StoryPropBuilder {
  ...
  #selectAnchor(platforms, definition, config, occupiedAnchorIds, usedAnchorIds) {
    ...
    const preferred = platforms.find(({ id }) => id === definition.preferredAnchorId);
    ...
    throw new RangeError(`Keine sichere Plattform fuer ${definition.type}.`);
  }
...
function clue(type, biomeId, targetY, preferredAnchorId, storyBeat) {
  return Object.freeze({
    type, biomeId, targetY, preferredAnchorId, storyBeat,
  });
}
