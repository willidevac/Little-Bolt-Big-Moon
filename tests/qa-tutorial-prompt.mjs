...
assert.match(html, /class="tutorial-prompt__label"/);
assert.match(html, /aria-expanded="true"/);
assert.match(css, /data-collapsed="true"/);
assert.match(css, /--tutorial-toggle-size:\s*2\.75rem/);
assert.match(css, /data-collapsed="true"[\s\S]*?width:\s*var\(--tutorial-toggle-size\)/);
assert.match(css, /data-collapsed="true"[\s\S]*?\.tutorial-prompt__label[\s\S]*?display:\s*none/);
